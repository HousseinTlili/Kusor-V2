...
@Component({
  ...
  template: `
    <div class="min-h-screen w-full bg-[#000000] text-slate-100 flex flex-col justify-between relative overflow-hidden font-sans">
      ...
      <!-- Center Content Area -->
      <div class="w-full max-w-md mx-auto px-6 py-10 relative z-20 flex-1 flex flex-col justify-center">
        <div class="p-8 sm:p-10 rounded-3xl bg-[#0A0A0A] border border-[#E85D04]/30 shadow-[0_0_50px_rgba(0,0,0,0.9)] backdrop-blur-2xl space-y-6">
          ...
          <div class="text-center space-y-3">
            <div class="inline-flex p-4 rounded-2xl bg-[#03071E] border border-[#E85D04]/30 shadow-inner justify-center items-center mb-1">
              <img src="assets/attijari_logo.png" alt="Attijari Bank" class="h-12 w-auto object-contain max-w-[200px]" />
            </div>
          ...
          <form (ngSubmit)="onSubmit()" class="space-y-5">
            <div>
              <label class="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-2">Identifiant Utilisateur</label>
              <div class="relative">
                <input type="text" [(ngModel)]="username" name="username" required placeholder="Ex: admin"
                  class="w-full pl-11 pr-4 py-3.5 rounded-xl bg-[#03071E] border border-slate-800 text-slate-100 placeholder-slate-600 focus:outline-none focus:border-[#E85D04] focus:ring-1 focus:ring-[#E85D04] transition-all text-sm font-medium" />
                <svg class="w-5 h-5 text-slate-500 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div>
              <label class="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-2">Mot de passe</label>
              <div class="relative">
            ...
            <button type="submit" [disabled]="loading()"
              class="w-full py-4 px-4 rounded-xl font-bold text-white bg-gradient-to-r from-[#E85D04] via-[#F48C06] to-[#DC2F02] hover:opacity-95 active:scale-[0.99] shadow-lg shadow-[#E85D04]/30 transition-all disabled:opacity-50 text-sm flex items-center justify-center space-x-2">
              @if (loading()) {
          ...
          <div class="pt-4 border-t border-slate-800/80">
            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-wider text-center mb-3">Rôles de démonstration</p>
            <div class="grid grid-cols-2 gap-2">
              <button (click)="fillPreset('admin', 'Admin123!')" type="button"
                class="px-3.5 py-2.5 rounded-xl bg-[#03071E] border border-slate-800 hover:border-[#E85D04]/60 text-xs font-semibold text-slate-300 hover:text-white transition-all text-left flex items-center justify-between">
                <span>Administrateur</span>
                <span class="text-[10px] px-1.5 py-0.5 rounded-md bg-[#E85D04]/20 text-[#E85D04] font-bold">Admin</span>
              </button>
              <button (click)="fillPreset('compliance_user', 'User123!')" type="button"
                class="px-3.5 py-2.5 rounded-xl bg-[#03071E] border border-slate-800 hover:border-[#E85D04]/60 text-xs font-semibold text-slate-300 hover:text-white transition-all text-left flex items-center justify-between">
                <span>Conformité</span>
                <span class="text-[10px] px-1.5 py-0.5 rounded-md bg-emerald-500/20 text-emerald-400 font-bold">User</span>
              </button>
            </div>
          </div>
      ...
      <!-- Footer Bar -->
      <footer class="w-full max-w-7xl mx-auto px-6 py-6 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 border-t border-slate-900/80 relative z-20 gap-2">
