...
class ObligationSearcher:
    ...
    def search(
        ...
        for i, rec in enumerate(records):
            c = rec.get("c") if hasattr(rec, "get") else rec["c"]
            c_props = dict(c) if hasattr(c, "items") else getattr(c, "_properties", {}) if hasattr(c, "_properties") else (c if isinstance(c, dict) else {})
            c_ref = c_props.get("reference", c_props.get("number", ""))

            o = rec.get("o") if hasattr(rec, "get") else rec["o"]
            o_props = dict(o) if hasattr(o, "items") else getattr(o, "_properties", {}) if hasattr(o, "_properties") else (o if isinstance(o, dict) else {})
